...
def main():
    ...
    all_values = []
    while opcode != 99:
        (opcode, values) = process()
        if len(values) == 3:
            all_values.append(values)

    print(all_values)
    max_x = max([val[0] for val in all_values])
    max_y = max([val[1] for val in all_values])
    print(max_x, max_y)
    l = [['.'] * max_x] * max_y
    result = l
    format_result()
    count_tiles_2 = sum([value[2] == 2 for value in all_values])
    result = count_tiles_2
