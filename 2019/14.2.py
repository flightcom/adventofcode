...
def produce_one_fuel(reactions, leftovers):
    elementary = False
    ...
    while not elementary:
        ...
        for i, (q, p) in enumerate(my_reaction):
            # print(q, p)
            q = int(q)
            print('need to produce', q, p)
            if p in primary_elements:
                print(p, 'in primary')
                continue
            else:
                print(p, 'not in primary')
    ...
    for key, value in final_elements.items():
        ...
        ore_used += quotient * int(reactions[index][0][0][0])

    return (ore_used, leftovers)
...
def main():
    ...
    ore_available = 1000000000

    fuel_produced = 0

    while ore_available > 0:
        (ore_used, leftovers) = produce_one_fuel(reactions, leftovers)
        fuel_produced += 1
        ore_available -= ore_used
        print(fuel_produced, ore_available, leftovers)
