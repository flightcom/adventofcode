...
def main():
    ...
    # print(len(input_data)/(image_dimensions['height']*image_dimensions['width']))
    ...
    count_digits = [{x: layer.count(str(x)) for x in range(10)} for layer in layers]
    count_digits = sorted(count_digits, key = lambda i: i[0]) 

    result = count_digits[0][2] * count_digits[0][1]
