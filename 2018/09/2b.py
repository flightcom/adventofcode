...
def main():
    ...
    last_marble = int(match.groups()[1])
    ...
    while iteration <= last_marble:
        ...
        if iteration % 23 != 0:
            ...
            if current_marble_position < len(marbles):
                # print('ok')
                marbles = {(x if x < current_marble_position else (
                    x+1)): y for x, y in marbles.items()}

                # keys = list(marbles.keys())
                # values = list(marbles.values())
                # print(keys)
                # print(values)
                # keys = [x if x < current_marble_position else (
                #     x+1) for x in keys]
                # keys.insert(current_marble_position, current_marble_position)
                # values.insert(current_marble_position, iteration)
                # marbles = dict(zip(keys, values))
            # else:
            marbles[current_marble_position] = iteration
        else:
            ...
            marbles = {i: y for i, (x, y) in enumerate(marbles.items())}
    ...
    # print(winning_player+1, max_score)
    # print(scores[winning_player])
    print(winning_player, scores[winning_player])
