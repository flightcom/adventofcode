    ...
    while iteration <= last_marble:
        ...
        if iteration % 23 != 0:
            current_marble_position = (
                current_marble_position + 1) % marbles.size + 1
            if current_marble_position == marbles.size:
                marbles = np.append(marbles, iteration)
            else:
                marbles = np.insert(
